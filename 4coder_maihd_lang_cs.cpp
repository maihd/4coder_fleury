...
internal void
F4_CS_ParseMacroDefinition(F4_Index_ParseCtx *ctx)
{
    Token *name = 0;
...
internal b32
F4_CS_SkipParseBody(F4_Index_ParseCtx *ctx)
{
    b32 body_found = 0;
    int nest = 0;
    ...
    for(;!ctx->done;)
    {
        ...
        else if(F4_Index_ParsePattern(ctx, "%b", TokenCSKind_PPDefine, &name))
        {
            F4_CS_ParseMacroDefinition(ctx);
        }
...
function b32
F4_CS_ParseDecl(F4_Index_ParseCtx *ctx, Token **name)
{
    Token *base_type = 0;
...
function void
F4_CS_ParseStructOrClassBodyIFuckingHateCPlusPlus(F4_Index_ParseCtx *ctx, F4_Index_NoteFlags note_flags)
{
    Token *name = 0;
    b32 valid = 0;
    b32 need_end_name = 0;
...
function b32
F4_CS_ParseFunctionBodyIFuckingHateCPlusPlus(F4_Index_ParseCtx *ctx, b32 *prototype_ptr)
{
    b32 valid = 0;
    b32 prototype = 0;
    ...
    for(;!ctx->done;)
    {
        ...
        if(token == 0) { break; }
        if(token->sub_kind == TokenCSKind_Semicolon)
        {
            valid = 1;
            prototype = 1;
            break;
        }
        else if(token->sub_kind == TokenCSKind_ParenCl)
        {
        }
...
function void
F4_CS_ParseEnumBodyIFuckingHateCPlusPlus(F4_Index_ParseCtx *ctx)
{
...
internal F4_LANGUAGE_INDEXFILE(F4_CS_IndexFile)
{
    int scope_nest = 0;
    for(b32 handled = 0; !ctx->done;)
    {
        ...
        {
            handled = 1;
            F4_CS_ParseStructOrClassBodyIFuckingHateCPlusPlus(ctx, F4_Index_NoteFlag_ProductType);
        }
        else if(F4_Index_ParsePattern(ctx, "%t%t", "typedef", "struct"))
        {
            handled = 1;
            F4_CS_ParseStructOrClassBodyIFuckingHateCPlusPlus(ctx, 0);
            if (F4_Index_ParsePattern(ctx, "%k", TokenBaseKind_Identifier, &name))
            {
        ...
        {
            handled = 1;
            F4_CS_ParseStructOrClassBodyIFuckingHateCPlusPlus(ctx, F4_Index_NoteFlag_SumType);
        }
                ...
                F4_Index_ParsePattern(ctx, "%t%t", "typedef", "enum"))
        {
            ...
            if(prototype == 0)
            {
                F4_CS_ParseEnumBodyIFuckingHateCPlusPlus(ctx);
            }
                                       ...
                                       "(")))
        {
            handled = 1;
            b32 prototype = 0;
            if(F4_CS_ParseFunctionBodyIFuckingHateCPlusPlus(ctx, &prototype))
            {
        ...
        else if(scope_nest == 0 && F4_CS_ParseDecl(ctx, &name))
        {
            handled = 1;
...
internal F4_LANGUAGE_POSCONTEXT(F4_CS_PosContext)
{
    int count = 0;
    ...
    // NOTE(rjf): Search for left parentheses (function call or macro invocation).
    {
        ...
        for(int i = 0; count < 4; i += 1)
        {
            ...
            if(token)
            {
                ...
                else if(token->sub_kind == TokenCSKind_ParenOp)
                {
                    paren_nest -= 1;
                }
                else if(token->sub_kind == TokenCSKind_ParenCl && i > 0)
                {
                    paren_nest += 1;
                }
                else if(token->sub_kind == TokenCSKind_Comma && i > 0 && paren_nest == 0)
                {
                    arg_idx += 1;
                }
            }
    ...
    // NOTE(rjf): Search for *.* pattern, or *->* pattern (accessing a type)
    {
        ...
        for(;;)
        {
            ...
            if(token)
            {
                ...
                else if((token->sub_kind == TokenCSKind_Dot ||
...
internal F4_LANGUAGE_HIGHLIGHT(F4_CS_Highlight)
{
}
